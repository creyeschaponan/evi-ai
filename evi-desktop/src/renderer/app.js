...
function setCoreState(state) {
  ...
  if (state === 'STANDBY') {
    ...
    if (currentSettings.listeningMode === 'alexa') {
      if (voicePromptText) voicePromptText.textContent = 'DI "EVI" O PRESIONA [ESPACIO] PARA HABLAR';
      if (voiceSubtext) voiceSubtext.textContent = 'Escucha continua activa // Modo Alexa operativo';
    } else {
      if (voicePromptText) voicePromptText.textContent = 'TOCA EL NÚCLEO O PRESIONA [ESPACIO] PARA HABLAR';
      if (voiceSubtext) voiceSubtext.textContent = 'Modo manual activo // Presiona para hablar';
    }
