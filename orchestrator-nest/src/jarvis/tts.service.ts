...
export class TtsService {
  ...
  cleanTextForSpeech(text: string): string {
    ...
    return text
      ...
      .replace(/\b(\d{1,2}):(\d{2})\s*(?:p\.?\s*m\.?|pm)\b/gi, '$1 y $2 de la tarde')
      .replace(/\b(\d{1,2}):(\d{2})\s*(?:a\.?\s*m\.?|am)\b/gi, '$1 y $2 de la mañana')
      .replace(/\b(\d{1,2}):(\d{2})\b/g, '$1 y $2')
      .replace(/\bp\.?\s*m\.?\b/gi, 'de la tarde')
      .replace(/\ba\.?\s*m\.?\b/gi, 'de la mañana')
      .replace(/[*_`#~()]/g, ' ')
      .replace(/https?:\/\/\S+/gi, '')
      .replace(/\s+/g, ' ')
      .trim();
